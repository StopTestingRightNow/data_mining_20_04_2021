...
def get_mut_following(init_username: str, client: Client, amount=20):
    ...
    followers_dict = client.user_followers(user_id, amount=20)
    following_dict = client.user_following(user_id, amount=20)
...
def find_chain(start_username: str, finish_username: str, client: Client, steps_limit=6):
    ...
    for i in range(1, steps_limit):
        ...
        for username in users_working_set:
            new_batch = get_mut_following(username, client=cl, amount=20)
...
if __name__ == "__main__":
    ...
    result = find_chain("ligapodlecov1", "ginstaboy", client=cl, steps_limit=6)
